...
if df is not None:
    ...
    invest_start = str(df.iloc[11, 15])
    invest_days = str(df.iloc[11, 17])

    def get_color_class(val):
        nums = parse_numeric(val)
        return 'up' if nums > 0 else 'down' if nums < 0 else 'flat'

    # --- 대시보드 메인 UI (Metric Cards & Table) ---
    st.markdown(f"""
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700&display=swap" rel="stylesheet">
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        html, body, [data-testid="stAppViewContainer"] {{ font-family: 'Noto Sans KR', sans-serif; background: #f5f5f3; color: #1a1a18; }}
        ...
        .total {{ font-size: 34px; font-weight: 700; color: #1a1a18; }}
        .metric-grid {{ display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 16px; margin-bottom: 28px; }}
        .metric-card {{ background: #fff; border: 1px solid rgba(0,0,0,0.05); border-radius: 16px; padding: 22px; }}
        .metric-label {{ font-size: 13.5px; color: #888780; margin-bottom: 10px; }}
        ...
        .stock-table th {{ color: #888780; padding: 14px 10px; text-align: right; border-bottom: 1.5px solid rgba(0,0,0,0.08); }}
    ...
    <div class="dash">
      <div class="header">
        <div><div style="font-size:14px; color:#888780;">총 자산 (Hstock V1.1)</div><div class="total">{int(real_total):,}원</div></div>
        <div style="text-align:right;"><div style="font-size:14px; color:#888780;">투자 시작일</div><div style="font-size:17px; font-weight:600;">{invest_start} | {invest_days}</div></div>
      </div>

      <div class="metric-grid">
        <div class="metric-card"><div class="metric-label">주식 평가금액</div><div class="metric-value">{int(sm['eval']):,}</div>원</div>
        <div class="metric-card"><div class="metric-label">매수금액</div><div class="metric-value">{int(sm['buy']):,}</div>원</div>
        <div class="metric-card"><div class="metric-label">누적 수익금</div><div class="metric-value {get_color_class(sm['profit'])}">{int(sm['profit']):+,} ({sm['rate']})</div>원</div>
        <div class="metric-card"><div class="metric-label">금일 변동액</div><div class="metric-value {get_color_class(sm['daily'])}">{int(sm['daily']):+,} ({sm['daily_rate']})</div>원</div>
        <div class="metric-card"><div class="metric-label">현금 보유량</div><div class="metric-value">{int(sm['cash']):,}</div>원</div>
      </div>

      <div class="card">
        <div style="font-size:18px; font-weight:700; margin-bottom:20px;">📦 보유 종목 현황</div>
        <table class="stock-table">
          <thead><tr><th style="text-align:left;">종목명</th><th>비중</th><th>현재가</th><th>평단가</th><th>주당전일비</th><th>금액전일비</th><th>수익금(수익률)</th></tr></thead>
          <tbody>
            {''.join([f"<tr><td style='text-align:left; font-weight:700;'>{r['Name']}</td><td>{r['Weight']}</td><td>{format_price(r['CurPrice'])}</td><td>{format_price(r['AvgPrice'])}</td><td class='{get_color_class(r['Diff'])}'>{format_price(r['Diff'])}</td><td class='{get_color_class(r['TotalDiff'])}'>{format_price(r['TotalDiff'])}</td><td><span class='badge {get_color_class(r['Profit'])}'>{format_price(r['Profit'])} ({r['Rate']})</span></td></tr>" for _, r in stocks.iterrows()])}
          </tbody>
        </table>
      </div>
    </div>
    ...
    <style>
        body {{ font-family: sans-serif; background: transparent; margin:0; }}
        .visual-container {{ display: flex; flex-direction: column; gap: 24px; padding: 24px; }}
    ...
    <div class="visual-container">
        ...
        <div class="card"><div class="card-title">💵 종목별 수익금 비교 (누적수익금)</div><div style="height:400px;"><canvas id="profitChart"></canvas></div></div>
    </div>
    <script>
        ...
        new Chart(document.getElementById('weightChart'), {{
            type: 'doughnut',
            data: {{ labels: {json.dumps(stock_names)}, datasets: [{{ data: {json.dumps(stock_weights)}, backgroundColor: palette, borderWidth: 0 }}] }},
        ...
        const pData = {json.dumps(stock_profits)};
        new Chart(document.getElementById('profitChart'), {{
            type: 'bar',
            data: {{ labels: {json.dumps(stock_names)}, datasets: [{{ label: '수익금', data: pData, backgroundColor: pData.map(v => v >= 0 ? '#D85A30' : '#3266AD'), borderRadius: 6 }}] }},
            options: {{ 
