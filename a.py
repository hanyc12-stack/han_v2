이 에러는 **`총 종목수(total_cnt)`가 0일 때** 발생합니다. 데이터가 아직 로드되지 않았거나 시트에 종목이 없을 경우를 대비해 **0으로 나누지 않도록 안전장치**를 추가하겠습니다.

### 🛠 수정 내용
*   `win_cnt/total_cnt` 대신 `(win_cnt/total_cnt*100 if total_cnt > 0 else 0)`와 같이 조건을 추가하여 에러를 방지합니다.
*   손실 종목 비율 계산 부분도 동일하게 수정합니다.

바로 코드를 수정해 드릴게요!

```python
# a.py 파일의 해당 부분을 아래와 같이 안전하게 수정했습니다.

# 수정 전:
# <div class="metric-sub up">{(win_cnt/total_cnt*100):.1f}%</div>

# 수정 후:
win_p = (win_cnt / total_cnt * 100) if total_cnt > 0 else 0
loss_p = (loss_cnt / total_cnt * 100) if total_cnt > 0 else 0

# 그리고 HTML 템플릿 안에서 이 변수(win_p, loss_p)를 사용하도록 변경합니다.
```
